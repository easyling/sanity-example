        ...
        rule
          .custom((label, context) => {
            ...
            if (context.parent?.isReference) {
              return true
            } else if (typeof label === 'undefined' || label.trim().length == 0) {
    ...
    defineField({
      ...
      validation: (rule) =>
        rule
          ...
          .error(),
      hidden: ({parent}) => parent?.action !== 'internal' || parent?.isReference,
    }),
    defineField({
      name: 'externalUrl',
      title: 'External URL',
      ...
      validation: (rule) =>
        rule
          ...
          .error(),
      hidden: ({parent}) =>
        parent?.isReference || (parent?.action !== 'external' && parent?.action !== 'video'),
    }),
    defineField({
      name: 'file',
      title: 'File',
      ...
      validation: (rule) =>
        rule
          ...
          .error(),
      hidden: ({parent}) => parent?.action !== 'file' || parent?.isReference,
    }),
    defineField({
      name: 'theme',
      title: 'Theme',
